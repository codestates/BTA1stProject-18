...
const SendTokens = () => {
  ...
  const currentWalletName = useSelector(({ walletEncrypted }) => walletEncrypted?.currentWalletName);
  ...
  return (
    <div>
      ...
      <input value={receiver} placeholder='Account ID 입력' onChange={(e) => setReceiver(e.target.value)} />
      <input value={amount} placeholder='금액 입력' onChange={(e) => setAmount(e.target.value)} type='number' />

      <>
        {loading ? <p>Loading...</p> : <button onClick={sendTransaction}>전송</button>}
